...
import { ArrowRight, Search, Star, Clock, TrendingUp, ShieldCheck, Hand, CalendarClock, BadgeCheck, BarChart3, MonitorSmartphone, Clapperboard, Cpu, BookOpen, type LucideIcon } from 'lucide-react'
...
export default function HomePage() {
  ...
  // Featured / latest guides for the "How to choose" homepage section.
    ...
    <div>
      ...
      {!filteredReviews && (
        <>
            ...
            <div className="container-page">
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <p className="mt-3 max-w-xl text-sm text-ink-500 dark:text-ink-400">
                    Before you compare specs and prices, learn the framework. These guides walk you
                    through what actually matters in each category — so you buy once, not twice.
                  </p>
                </div>
                <Link to="/guides" className={sectionJumpCls}>
                  All buyer's guides
              ...
              <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {featuredGuides.map((g) => {
                  const tag = g.tags[0]
                  const meta = categories.find((c) => c.slug === tag)
                  const Icon = categoryIcons[tag as CategorySlug] ?? BookOpen
                  return (
                    <Link
                      key={g.slug}
                      to={`/guides/${g.slug}`}
                      className="group flex flex-col overflow-hidden rounded-2xl border border-ink-200/80 bg-white shadow-sm transition-all hover:-translate-y-1 hover:border-accent-300 hover:shadow-lg dark:border-ink-700/80 dark:bg-ink-950 dark:shadow-none dark:hover:border-accent-700"
                    >
                      {/* Cover / subtle icon header */}
                      <div className="flex h-32 items-center justify-center bg-gradient-to-br from-accent-50 to-ink-50 dark:from-accent-500/10 dark:to-ink-900">
                        <div className={`flex h-14 w-14 items-center justify-center rounded-2xl ${meta?.accentClass ?? 'text-accent-600 bg-accent-50 dark:bg-accent-500/10'}`}>
                      ...
                      <div className="flex flex-1 flex-col p-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(g.tags ?? []).map((t) => {
                            const m = categories.find((c) => c.slug === t)
                            return (
                              <span
                                key={t}
                                className="rounded-full bg-ink-100 px-2.5 py-0.5 text-xs font-semibold text-ink-600 dark:bg-ink-800 dark:text-ink-300"
                              >
                                #{m?.name ?? t}
                              </span>
                            )
                        ...
                        <h3 className="mt-3 font-serif text-lg font-semibold text-ink-900 dark:text-ink-100">
                          {g.title}
                        </h3>
                        ...
                        <div className="mt-5 flex items-center justify-between">
                          ...
                          <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-accent-700 group-hover:text-accent-900 dark:text-accent-400 dark:group-hover:text-accent-300">
                            Read Guide
          ...
          <section className="bg-white py-20 lg:py-28 dark:bg-ink-950">
            <div className="container-page">
              <div className="flex items-end justify-between gap-4">
                ...
                <Link to="/comparisons" className={sectionJumpCls}>
                  All comparisons
                  <ArrowRight className="h-3.5 w-3.5" />
                </Link>
              </div>
          ...
          <section className="bg-ink-50 py-20 lg:py-28 dark:bg-ink-900">
            <div className="container-page">
              <div className="flex items-end justify-between gap-4">
                ...
                <Link to="/reviews" className={sectionJumpCls}>
                  All reviews
                  <ArrowRight className="h-3.5 w-3.5" />
                </Link>
              </div>
          ...
          {/* Category deep-dive blocks */}
          {categories.map((category) => {
            const catReviews = reviews.filter((r) => r.category === category.slug)
            if (catReviews.length === 0) return null

            return (
              <section key={category.slug} className="bg-ink-50 py-16 lg:py-24 dark:bg-ink-900">
                <div className="container-page">
                  <div className="flex items-end justify-between gap-4">
                    <div>
                    ...
                    <Link to={`/reviews/${category.slug}`} className={sectionJumpCls}>
                      View all
                      <ArrowRight className="h-3.5 w-3.5" />
                    </Link>
                  </div>
          ...
          {/* Editorial values strip — "Why trust us" dark band */}
          <section className="bg-ink-900 py-20 lg:py-28 text-white">
            <div className="container-page">
